# Rex subconverter build

This fork keeps upstream subconverter source intact by default and applies a small build-time patch for personal Mihomo fallback usage.

## What this build changes

`patches/rex-fallback-fields.patch` extends `custom_groups` TOML handling for `type = "fallback"` so these fields are parsed and emitted into the generated Clash/Mihomo YAML:
...
```bash
docker build -f Dockerfile.rex -t subconverter:rex .
docker run --rm -p 25500:25500 subconverter:rex
curl http://127.0.0.1:25500/version
```

## Build with GitHub Actions

Run **Actions → Build Rex Docker Image → Run workflow**.
...
```text
ghcr.io/moekyo/subconverter:rex
...
services:
  subconverter:
    image: ghcr.io/moekyo/subconverter:rex
